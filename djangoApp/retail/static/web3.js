...
const contractAddress = "0xB4BE7ba6E93C7C07695C00bffEF1576b5d7aDba1";
...
async function payOrder(element) {
    ...
    const priceInWei = web3.utils.toWei(orderPriceETH, 'ether');
    ...
    try {
        ...
        await contract.methods.pay(orderId).send({
...
async function confirmDelivered(element) {
  ...
  try {
      ...
      await contract.methods.markDelivered(orderId).call({
          from: account, // The user's account
      });
